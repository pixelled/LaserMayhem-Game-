...
$(function() {
    ...
    var spreadship_img = new Image();
    var smashship_img = new Image();
    var direct_img = new Image();
    ...
    spreadship_img.src = "assets/img/spreadship.png";
    direct_img.src = "assets/img/direct.png";
    ...
    $(".card").click(function(e) {
        choice = e.target.id.slice(-2);
        $("#ship_ab").css("border-color", "#000");
        $("#ship_cd").css("border-color", "#000");
        $("#ship_ef").css("border-color", "#000");
        $("#ship_" + choice).css("border-color", "#FFA000");
    })
...
class SpreadPlayer extends Playership {
    ...
    drawShip(ctx) {
        ...
        ctx.rotate(this.dir.angle());
        ctx.shadowColor = "#0000ff";
        ctx.drawImage(spreadship_img, -30, -30);
        ctx.restore();
    }
...
class SpinPlayer extends Playership {
    ...
    drawShip(ctx) {
        ...
        ctx.rotate(this.dir.angle());
        ctx.shadowColor = "#0000ff";
        ctx.drawImage(smashship_img, -30, -30);
        ctx.restore();
    }

    shoot() {
        if ((keysPressed[code(' ')] && now_time - this.firedAt > this.load)) {
            this.lasers.unshift(new RoundLaser(this.dir.copy().normalize(), this.pos.copy(), 1, "#0000ff", 20));
            this.firedAt = now_time;
        }
    }
}
...
class DirectPlayer extends Playership {
    ...
    drawShip(ctx) {
        ...
        ctx.rotate(this.dir.angle());
        ctx.shadowColor = "#0000ff";
        ctx.drawImage(direct_img, -30, -30);
        ctx.restore();
    }

    shoot(now_time) {
        if ((keysPressed[code(' ')] && now_time - this.firedAt > this.load)) {
            this.lasers.unshift(new UpLaser(this.dir.copy().normalize(), this.pos.copy(), 1, "#0000ff", 10));
            this.lasers.unshift(new Laser(this.dir.copy().rotate(Math.PI / 9).normalize(), this.pos.copy(), 10, "#0000ff"));
            this.lasers.unshift(new Laser(this.dir.copy().rotate(-Math.PI / 9).normalize(), this.pos.copy(), 10, "#0000ff"));
            this.firedAt = now_time;
        }
    }
...
var last_update = new Date().getTime();
    var ship_one = new TestShip(new Vector(w * 1.2, h * 1.2), new Vector(0, -0.3));
    var first = new Space("display", ship_one, [new Asteroid(new Vector(-w, h * 3 / 4), new Vector(-2, 2), new Vector(1, -1), 10),
                                                new Asteroid(new Vector(w * 1 / 3, h), new Vector(1, 6), new Vector(-1, 0.7), 10),
                                                new Sniper(new Vector(-w * Math.random(), -h), new Vector(1, 1))],
                                               [new NormalShip(new Vector(-w, h * Math.random()), new Vector(-0.2, -0.3)),
                                                new Spread(new Vector(w, h * Math.random()), new Vector(3, 1)),
                                                new Asteroid(new Vector(w * Math.random(), h), new Vector(3, 4), new Vector(-1, 0.1), 10)]);
    first.createFrame();
